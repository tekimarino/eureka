...
# Aliases utilisés plus bas dans ce fichier (ancien code)
_db_ensure_ready = ensure_store_ready
_kv_get = kv_get
_kv_set = kv_set
_kv_delete = kv_delete
_kv_keys = kv_keys
...
# --- Storage mode ------------------------------------------------------------
# If DATABASE_URL is set, we persist ALL app data in PostgreSQL (public.kv_store)
# so deployments don't wipe data.
DB_MODE = _db_enabled()
...
def _storage_key(file_path: str) -> str:
    """Map a local JSON filename to a DB key.

    Example: data/users.json -> users
    """
...
# Ensure DB table exists as soon as the app starts (in DB mode)
if DB_MODE:
    _db_ensure_ready()
...
def _load_json(path: str, default: Any) -> Any:
    ...
    When DATABASE_URL is set (DB_MODE), this reads from PostgreSQL table
    public.kv_store (k/v JSONB). This prevents data loss across deployments.
    """
    if DB_MODE:
        ...
        # Seed the DB from the repo's data/*.json (if present), otherwise use default.
        seeded = default
        try:
...
def _save_json(path: str, data: Any) -> None:
    if DB_MODE:
        _kv_set(_storage_key(path), data)
        return
...
def _ensure_data_files() -> None:
    ...
    # In DB mode, we seed the PostgreSQL kv_store once (if the key is missing)
    # and we DO NOT depend on the local JSON files for runtime persistence.
    if DB_MODE:
        seed_defaults = {
            ZONES_FILE: [],
            CENTERS_FILE: [],
            ...
            PAYROLL_FILE: [],
            SETTINGS_FILE: {
                "pay_per_approved": 250,
                "reject_penalty": 0,
                "bonus_threshold": 50,
                "bonus_amount": 0,
                "fixed_bonus_biweekly": 10000,
            },
            AUDIT_LOG_FILE: [],
            SMS_CONFIG_FILE: {"enabled": False, "provider": "", "api_key": "", "sender": ""},
            SMS_CAMPAIGNS_FILE: [],
            SMS_OUTBOX_FILE: [],
            SMS_LOGS_FILE: [],
        }

        for path, default in seed_defaults.items():
            key = _storage_key(path)
            if _kv_get(key) is not None:
                continue
            # Prefer the repo's JSON if present (lets you ship initial data)
            val = default
...
# Run initialization at import time (Gunicorn/DO App Platform doesn't
...
def admin_reset_data():
    ...
    with zipfile.ZipFile(backup_zip, "w", zipfile.ZIP_DEFLATED) as z:
        targets = [USERS_FILE, REG_FILE, SETTINGS_FILE, PAYROLL_FILE, APPROVALS_QUEUE_FILE, CENTERS_FILE, ZONES_FILE]
        if DB_MODE:
            for p in targets:
                key = _storage_key(p)
                val = _kv_get(key)
                if val is None:
                    continue
                z.writestr(os.path.basename(p), json.dumps(val, ensure_ascii=False, indent=2))
        else:
            for p in targets:
                if os.path.exists(p):
...
def admin_backup():
    ...
    with zipfile.ZipFile(out_path, "w", compression=zipfile.ZIP_DEFLATED) as z:
        if DB_MODE:
            # Export DB snapshots as data/*.json inside the zip
            with tempfile.TemporaryDirectory() as tmpdir:
                tmp_data = os.path.join(tmpdir, "data")
                os.makedirs(tmp_data, exist_ok=True)

                for file_path in DATA_FILES:
                    key = _storage_key(file_path)
                    val = _kv_get(key, None)
                    if val is None:
                        continue
                    out_fn = os.path.join(tmp_data, f"{key}.json")
                    with open(out_fn, "w", encoding="utf-8") as f:
                        json.dump(val, f, ensure_ascii=False, indent=2)
                    z.write(out_fn, arcname=f"data/{key}.json")

                # uploads are still file-based
                for fn in os.listdir(UPLOADS_DIR):
                    z.write(os.path.join(UPLOADS_DIR, fn), arcname=f"uploads/{fn}")
        else:
            # File mode: zip the data/ folder
    ...
    if request.method == "POST":
        ...
        try:
            with zipfile.ZipFile(tmp, "r") as z:
                ...
                if DB_MODE:
                    # Restore JSON snapshots into PostgreSQL kv_store
                    if os.path.isdir(data_src):
                        for fn in os.listdir(data_src):
                            if not fn.lower().endswith(".json"):
                                continue
                            key = os.path.splitext(fn)[0]
                            with open(os.path.join(data_src, fn), "r", encoding="utf-8") as f:
                                val = json.load(f)
                            _kv_set(key, val)
                else:
                    # File mode: overwrite data/*.json
                    if os.path.isdir(data_src):
                        for fn in os.listdir(data_src):
                            if fn.lower().endswith(".json"):
                                shutil.copy2(os.path.join(data_src, fn), os.path.join(DATA_DIR, fn))

                # uploads are still file-based in both modes
